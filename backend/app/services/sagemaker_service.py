...
import boto3
from botocore.exceptions import ClientError, BotoCoreError, ConnectionError, ReadTimeoutError
...
class SageMakerService:
    ...
    def _retry_invoke_endpoint(
        ...
        for attempt in range(max_retries + 1):  # +1 for initial attempt
            try:
                ...
                result['response_size'] = len(response_body) if isinstance(response_body, bytes) else len(response_body.encode('utf-8'))
                ...
                last_error = e
                logger.error(
                    "SageMaker endpoint unexpected error",
                    attempt=attempt + 1,
                    error=str(e)
                )
        ...
        try:
            ...
            try:
                ...
                if not 0 <= confidence <= 1:
                    logger.warning(
                        "Confidence score out of expected range",
    ...
    async def get_endpoint_info(self, endpoint_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise SageMakerServiceError(f"Failed to get endpoint info: {e.response['Error']['Message']}")
    ...
    async def get_endpoint_config(self, config_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SageMakerServiceError(f"Failed to get endpoint config: {e.response['Error']['Message']}")
    ...
    async def disable_endpoint(
        ...
        try:
            ...
            logger.info(
                "Starting endpoint disable process",
                endpoint_name=target_endpoint
            )
            ...
            if current_instances == 0:
                logger.info(
                    "Endpoint already has 0 instances",
                    endpoint=target_endpoint
                )
                return {
                    "endpoint_name": target_endpoint,
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'ValidationException' and 'already exists' in str(e):
                    logger.info("Disabled configuration already exists", config_name=disabled_config_name)
                else:
                    raise
            ...
            if wait_for_completion:
                ...
                while time.time() - start_time < timeout:
                    try:
                        ...
                        logger.warning("Error checking endpoint status during disable", error=str(e))
                        time.sleep(30)

                # Timeout reached
                logger.error(
                    "Endpoint disable timeout",
                    endpoint=target_endpoint,
                    timeout=timeout
                )
                return {
    ...
    async def reenable_endpoint(
        ...
        try:
            ...
            target_instances = sum(
                variant["InitialInstanceCount"] for variant in target_config_info["production_variants"]
            )
            ...
            if wait_for_completion:
                ...
                while time.time() - start_time < timeout:
                    try:
                        ...
                        logger.warning(
                            "Error checking endpoint status during re-enable",
                            error=str(e)
                        )
                        time.sleep(30)
            ...
            logger.error(
                "Failed to re-enable endpoint",
                endpoint_name=endpoint_name,
                error=str(e)
            )
            raise SageMakerServiceError(f"Failed to re-enable endpoint: {str(e)}")

    async def list_endpoints(self, status_filter: Optional[str] = None) -> Dict[str, Any]:
        """
