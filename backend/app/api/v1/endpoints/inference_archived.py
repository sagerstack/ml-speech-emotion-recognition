"""
DEPRECATED: This endpoint is archived and no longer maintained.

This file contained the SageMaker-based inference endpoints which have been
replaced by local model inference in inference.py (previously inference_local.py).

Routes that were here:
- POST /v1/infer/sagemaker/latest
- POST /v1/infer/sagemaker/{version}
- POST /v1/infer/sagemaker/all

Do not use these endpoints. Use /v1/inference/* endpoints instead.

Original documentation:
Inference API endpoints for ML Speech Emotion Recognition.

This module provided endpoints for real-time emotion prediction using
...
from app.infrastructure.audio.processor import AudioProcessingError, audio_processor
...
INFERENCE_REQUESTS = Counter(
    "inference_requests_total", "Total inference requests", ["emotion", "success"]
)

INFERENCE_DURATION = Counter(
    "inference_duration_seconds", "Inference request duration", ["endpoint"]
)

SAGEMAKER_INVOCATIONS = Counter(
    "sagemaker_invocations_total", "Total SageMaker endpoint invocations", ["endpoint", "status"]
)


@router.post("/infer")
async def infer_emotion(file: UploadFile = File(...), user_id: str = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        is_valid, error_message = validate_audio_file(file_content, file.content_type, max_size_mb)
        ...
        try:
            audio_base64, sample_rate = audio_processor.audio_to_base64(file_content, file.filename)
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Audio processing failed: {str(e)}"
            )
        ...
        try:
            ...
            prediction_result = await sagemaker_service.predict_emotion(
                audio_base64=audio_base64, sample_rate=sample_rate
            )

            SAGEMAKER_INVOCATIONS.labels(
                endpoint=settings.sagemaker_endpoint_name, status="success"
            ).inc()
            ...
            SAGEMAKER_INVOCATIONS.labels(
                endpoint=settings.sagemaker_endpoint_name, status="connection_error"
            ).inc()
            INFERENCE_REQUESTS.labels(emotion="unknown", success="false").inc()
            raise HTTPException(
            ...
            SAGEMAKER_INVOCATIONS.labels(
                endpoint=settings.sagemaker_endpoint_name, status="prediction_error"
            ).inc()
            INFERENCE_REQUESTS.labels(emotion="unknown", success="false").inc()
            raise HTTPException(
            ...
            SAGEMAKER_INVOCATIONS.labels(
                endpoint=settings.sagemaker_endpoint_name, status="service_error"
            ).inc()
            INFERENCE_REQUESTS.labels(emotion="unknown", success="false").inc()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def inference_health_check() -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "unhealthy", "service": "emotion-inference", "error": str(e)}
