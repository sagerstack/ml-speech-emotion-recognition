...
def extract_features(audio_bytes: bytes, filename: str) -> np.ndarray:
    ...
    try:
        ...
        data, sr = librosa.load(io.BytesIO(audio_bytes), duration=2.5, offset=0.6)
...
def extract_features_with_viz(audio_bytes: bytes, filename: str) -> dict:
    """
    ...
    captures intermediate matrices and means for frontend audio_features visualization.
    ...
    Returns:
        Dict with two keys:
            ...
            "audio_features": Dict containing:
                ...
                - "mel_spectrogram": List[List[float]] - shape (64, T) - matches frontend
                - "chroma": List[List[float]] - shape (12, T)
                - "mfcc_means": List[float] - shape (20,) - for equalizer charts
                - "delta_mfcc_means": List[float] - shape (20,) - for delta charts
                - "delta_delta_mfcc_means": List[float] - shape (20,) - for delta2 charts
                - "mfcc": List[List[float]] - shape (20, T) - full matrix
                - "delta_mfcc": List[List[float]] - shape (20, T) - full matrix
                - "delta_delta_mfcc": List[List[float]] - shape (20, T) - full matrix
    ...
    try:
        ...
        # Also compute 64-band mel spectrogram for frontend compatibility
        mel_64 = librosa.feature.melspectrogram(y=data, sr=sr, n_mels=64)  # (64, T)
        ...
        audio_features_data = {
            ...
            # Spectral features (matrices) - 64 bands for frontend compatibility
            "mel_spectrogram": mel_64.tolist(),  # (64, T) - matches frontend
            "chroma": chroma_stft.tolist(),  # (12, T)
            # MFCC means for frontend equalizer charts
            "mfcc_means": mfcc_mean.tolist(),  # (20,) - averaged for charts
            "delta_mfcc_means": delta_mfcc_mean.tolist(),  # (20,) - averaged for charts
            "delta_delta_mfcc_means": delta2_mfcc_mean.tolist(),  # (20,) - averaged for charts
            # Full matrices (keep for advanced use)
