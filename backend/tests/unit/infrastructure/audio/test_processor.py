...
from app.infrastructure.audio.processor import (
    AudioFeatures,
    AudioMetadata,
...
class TestAudioMetadata:
    ...
    def test_audio_metadata_duration_validation(self) -> None:
        ...
        for duration in valid_durations:
            metadata = AudioMetadata(
                duration=duration, sample_rate=22050, channels=1, format="wav", file_size_bytes=1000
            )
        ...
        for duration in invalid_durations:
            with pytest.raises(
                ValidationError, match="Duration must be positive|Audio duration cannot exceed"
            ):
                AudioMetadata(
                    duration=duration,
                    sample_rate=22050,
    ...
    def test_audio_metadata_edge_cases(self) -> None:
        ...
        metadata = AudioMetadata(
            duration=0.01, sample_rate=22050, channels=1, format="wav", file_size_bytes=1
        )
        assert metadata.duration == 0.01

        # Maximum valid duration
        metadata = AudioMetadata(
            duration=60.0, sample_rate=22050, channels=1, format="wav", file_size_bytes=1000000
        )
...
class TestFeaturesToDict:
    ...
    def test_features_to_dict_preserves_data_integrity(
        self, sample_audio_features: AudioFeatures
    ) -> None:
        ...
        assert (
            features_dict["metadata"]["sample_rate"] == sample_audio_features.metadata.sample_rate
        )
        ...
        assert (
            len(features_dict["mfcc"][0]) == sample_audio_features.mfcc.shape[1]
            if sample_audio_features.mfcc.ndim > 1
            else True
        )
...
class TestGlobalAudioProcessor:
    ...
    def test_global_audio_processor_instance(self) -> None:
        ...
        from app.infrastructure.audio.processor import audio_processor
    ...
    def test_global_processor_singleton_behavior(self) -> None:
        ...
        from app.infrastructure.audio.processor import audio_processor as processor2
