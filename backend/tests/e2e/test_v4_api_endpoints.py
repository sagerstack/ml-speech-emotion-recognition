"""
End-to-End tests for v4 Model API Endpoints.

This test suite validates the complete API flow for v4 model endpoints:
- POST /v1/infer/local/4 - Inference with v4 model
...
v4 Model Details:
- Same architecture as v3 (Ultra Ensemble)
- Same feature dimension (210 features)
- Trained on combined CREMA-D + RAVDESS datasets
- Endpoint: /v1/infer/local/4
"""
...
@pytest.mark.e2e
class TestV4InferenceEndpoint:
    """Test POST /v1/infer/local/4 endpoint."""

    def test_v4_inference_endpoint_returns_200(self, client, sample_audio_file):
        """Test that v4 inference endpoint returns 200 OK."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }

        response = client.post('/v1/infer/local/4', files=files)

        assert response.status_code == 200, \
            f"Expected 200, got {response.status_code}: {response.text}"

    def test_v4_inference_response_structure(self, client, sample_audio_file):
    ...
    def test_v4_inference_prediction_structure(self, client, sample_audio_file):
    ...
    def test_v4_inference_model_info_structure(self, client, sample_audio_file):
    ...
    def test_v4_inference_version_correct(self, client, sample_audio_file):
        """Test that response version is '4'."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert data["version"] == "4", \
            f"Version should be '4', got {data['version']}"

    def test_v4_inference_emotion_valid(self, client, sample_audio_file):
    ...
    def test_v4_inference_confidence_valid_range(self, client, sample_audio_file):
    ...
    def test_v4_inference_all_probabilities_valid(self, client, sample_audio_file):
        """Test that all_probabilities contains all emotion classes.

        Note: v4 model uses Ultra Ensemble with probability support,
        so it returns probabilities for all classes.
        """
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
    ...
    def test_v4_inference_probabilities_sum_to_one(self, client, sample_audio_file):
    ...
    def test_v4_inference_model_type_correct(self, client, sample_audio_file):
        """Test that model type is Ultra Ensemble."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert model_type == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)", \
            f"Model type should be Ultra Ensemble, got {model_type}"

    def test_v4_inference_features_used_correct(self, client, sample_audio_file):
        """Test that features_used is 210."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert features_used == 210, \
            f"Features used should be 210, got {features_used}"

    def test_v4_inference_processing_time_reasonable(self, client, sample_audio_file):
...
@pytest.mark.e2e
@pytest.mark.audio
class TestV4RealAudioEndpoint:
    """Test endpoint with real CREMA-D audio files."""

    def test_v4_inference_with_real_crema_d_audio(self, client, crema_d_sample):
    ...
    def test_v4_inference_multiple_real_files(self, client, crema_d_audio_path):
...
class TestV4ErrorHandling:
    ...
    def test_v4_inference_with_corrupted_file(self, client, corrupted_audio_file):
    ...
    def test_v4_inference_with_empty_file(self, client):
    ...
    def test_v4_inference_without_file(self, client):
        """Test that request without file returns 422 validation error."""
        response = client.post('/v1/infer/local/4')

        assert response.status_code == 422, \
            f"Request without file should return 422, got {response.status_code}"

    def test_v4_inference_with_invalid_file_type(self, client):
...
@pytest.mark.e2e
class TestV4FileFormatSupport:
    """Test support for different audio file formats."""

    def test_v4_inference_with_wav_file(self, client, sample_audio_file):
    ...
    def test_v4_inference_with_different_extensions(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV4EndpointConsistency:
    """Test endpoint consistency and reliability."""

    def test_v4_multiple_requests_same_audio(self, client, sample_audio_file):
    ...
    def test_v4_concurrent_requests_work(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV4ModelInfoEndpoints:
    """Test model info endpoints for v4."""

    def test_get_model_info_v4(self, client):
        """Test GET /v1/models/local/4/info endpoint."""
        response = client.get('/v1/models/local/4/info')
        ...
        # Check values
        assert data["version"] == "4", "Version should be '4'"
        ...
        assert data["feature_dimension"] == 210, "Feature dimension should be 210"
        assert len(data["classes"]) == 6, "Should have 6 classes"

    def test_list_models_includes_v4(self, client):
        """Test GET /v1/models/local/list includes v4."""
        ...
        # Check v4 is in the list
        version_numbers = [v["version"] for v in data["versions"]]
        assert "4" in version_numbers, "v4 should be in versions list"

        # Find v4 in the list
        v4_info = next((v for v in data["versions"] if v["version"] == "4"), None)
        assert v4_info is not None, "v4 info should be in list"
        assert v4_info["model_type"] == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)"
        assert v4_info["feature_dimension"] == 210


@pytest.mark.e2e
class TestV4VsOtherVersionsComparisonEndpoint:
    """Test comparison between v4 and other version endpoints."""

    def test_same_audio_different_versions_all_work(self, client, sample_audio_file):
        """Test that same audio works with v1, v2, v3, and v4 endpoints."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
        ...
        response_v3 = client.post('/v1/infer/local/3', files=files_v3)
        response_v4 = client.post('/v1/infer/local/4', files=files_v4)

        # All should succeed
        assert response_v1.status_code == 200, "v1 endpoint should work"
        assert response_v2.status_code == 200, "v2 endpoint should work"
        assert response_v3.status_code == 200, "v3 endpoint should work"
        assert response_v4.status_code == 200, "v4 endpoint should work"
        ...
        assert data_v3["prediction"]["emotion"] in valid_emotions
        assert data_v4["prediction"]["emotion"] in valid_emotions

    def test_v4_uses_same_features_as_v3(self, client, sample_audio_file):
        """Test that v4 uses same feature count as v3 (210 features)."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
        ...
        # Different feature dimensions for v1 and v2, same for v3 and v4
        ...
        assert data_v3["model_info"]["features_used"] == 210, "v3 should use 210 features"
        assert data_v4["model_info"]["features_used"] == 210, "v4 should use 210 features (same as v3)"

    def test_v4_has_same_model_type_as_v3(self, client, sample_audio_file):
        """Test that v4 has same model type as v3 (Ultra Ensemble)."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
        ...
        # Different model types for v1 and v2, same for v3 and v4
        assert data_v1["model_info"]["type"] == "DecisionTreeClassifier"
        assert data_v2["model_info"]["type"] == "Pipeline (StandardScaler + RFE + SVC)"
        assert data_v3["model_info"]["type"] == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)"
        assert data_v4["model_info"]["type"] == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)"


@pytest.mark.e2e
class TestV4LatestEndpoint:
    """Test latest model endpoint (when v4 is latest)."""

    def test_latest_endpoint_returns_v4_or_higher(self, client, sample_audio_file):
        """Test POST /v1/infer/local/latest endpoint returns v4 or higher."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        # Version should be 4 or higher
        version = int(data["version"])
        assert version >= 4, \
            f"Latest version should be >= 4, got {version}"
