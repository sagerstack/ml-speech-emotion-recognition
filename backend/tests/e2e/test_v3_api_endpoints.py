"""
End-to-End tests for v3 Model API Endpoints.

This test suite validates the complete API flow for v3 model endpoints:
- POST /v1/infer/local/3 - Inference with v3 model
...
@pytest.mark.e2e
class TestV3InferenceEndpoint:
    """Test POST /v1/infer/local/3 endpoint."""

    def test_v3_inference_endpoint_returns_200(self, client, sample_audio_file):
        """Test that v3 inference endpoint returns 200 OK."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }

        response = client.post('/v1/infer/local/3', files=files)

        assert response.status_code == 200, \
            f"Expected 200, got {response.status_code}: {response.text}"

    def test_v3_inference_response_structure(self, client, sample_audio_file):
    ...
    def test_v3_inference_prediction_structure(self, client, sample_audio_file):
    ...
    def test_v3_inference_model_info_structure(self, client, sample_audio_file):
    ...
    def test_v3_inference_version_correct(self, client, sample_audio_file):
        """Test that response version is '3'."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert data["version"] == "3", \
            f"Version should be '3', got {data['version']}"

    def test_v3_inference_emotion_valid(self, client, sample_audio_file):
    ...
    def test_v3_inference_confidence_valid_range(self, client, sample_audio_file):
    ...
    def test_v3_inference_all_probabilities_valid(self, client, sample_audio_file):
        """Test that all_probabilities contains all emotion classes.

        Note: v3 model uses Ultra Ensemble with probability support,
        ...
        # v3 model should return probabilities for all classes
    ...
    def test_v3_inference_probabilities_sum_to_one(self, client, sample_audio_file):
    ...
    def test_v3_inference_model_type_correct(self, client, sample_audio_file):
    ...
    def test_v3_inference_features_used_correct(self, client, sample_audio_file):
    ...
    def test_v3_inference_processing_time_reasonable(self, client, sample_audio_file):
...
@pytest.mark.e2e
@pytest.mark.audio
class TestV3RealAudioEndpoint:
    """Test endpoint with real CREMA-D audio files."""

    def test_v3_inference_with_real_crema_d_audio(self, client, crema_d_sample):
    ...
    def test_v3_inference_multiple_real_files(self, client, crema_d_audio_path):
...
@pytest.mark.e2e
class TestV3ErrorHandling:
    """Test error handling through API endpoints."""

    def test_v3_inference_with_corrupted_file(self, client, corrupted_audio_file):
    ...
    def test_v3_inference_with_empty_file(self, client):
    ...
    def test_v3_inference_without_file(self, client):
        """Test that request without file returns 422 validation error."""
        response = client.post('/v1/infer/local/3')

        assert response.status_code == 422, \
            f"Request without file should return 422, got {response.status_code}"

    def test_v3_inference_with_invalid_file_type(self, client):
...
@pytest.mark.e2e
class TestV3FileFormatSupport:
    """Test support for different audio file formats."""

    def test_v3_inference_with_wav_file(self, client, sample_audio_file):
    ...
    def test_v3_inference_with_different_extensions(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV3EndpointConsistency:
    """Test endpoint consistency and reliability."""

    def test_v3_multiple_requests_same_audio(self, client, sample_audio_file):
    ...
    def test_v3_concurrent_requests_work(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV3ModelInfoEndpoints:
    """Test model info endpoints for v3."""

    def test_get_model_info_v3(self, client):
        """Test GET /v1/models/local/3/info endpoint."""
        response = client.get('/v1/models/local/3/info')
        ...
        # Check values
        assert data["version"] == "3", "Version should be '3'"
    ...
    def test_list_models_includes_v3(self, client):
        """Test GET /v1/models/local/list includes v3."""
        ...
        # Check v3 is in the list
        version_numbers = [v["version"] for v in data["versions"]]
        assert "3" in version_numbers, "v3 should be in versions list"

        # Find v3 in the list
        v3_info = next((v for v in data["versions"] if v["version"] == "3"), None)
        assert v3_info is not None, "v3 info should be in list"
        assert v3_info["model_type"] == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)"
        assert v3_info["feature_dimension"] == 210


@pytest.mark.e2e
class TestV3VsOtherVersionsComparisonEndpoint:
    """Test comparison between v3 and other version endpoints."""

    def test_same_audio_different_versions_all_work(self, client, sample_audio_file):
        """Test that same audio works with v1, v2, and v3 endpoints."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
    ...
    def test_v3_uses_different_features_than_v1_and_v2(self, client, sample_audio_file):
        """Test that v3 uses different feature count than v1 and v2."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
    ...
    def test_v3_has_different_model_type_than_v1_and_v2(self, client, sample_audio_file):
        """Test that v3 has different model type than v1 and v2."""
        files_v1 = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        files_v2 = {
...
@pytest.mark.e2e
class TestV3LatestEndpoint:
    """Test latest model endpoint (when v3 is latest)."""

    def test_latest_endpoint_returns_v3_or_higher(self, client, sample_audio_file):
        """Test POST /v1/infer/local/latest endpoint returns v3 or higher."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert version >= 3, \
            f"Latest version should be >= 3, got {version}"
