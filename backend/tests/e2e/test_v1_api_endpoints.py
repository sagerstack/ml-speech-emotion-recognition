"""
End-to-End tests for v1 Model API Endpoints.

This test suite validates the complete API flow for v1 model endpoints:
- POST /v1/infer/local/1 - Inference with v1 model
...
@pytest.mark.e2e
class TestV1InferenceEndpoint:
    """Test POST /v1/infer/local/1 endpoint."""

    def test_v1_inference_endpoint_returns_200(self, client, sample_audio_file):
        """Test that v1 inference endpoint returns 200 OK."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }

        response = client.post('/v1/infer/local/1', files=files)

        assert response.status_code == 200, \
            f"Expected 200, got {response.status_code}: {response.text}"

    def test_v1_inference_response_structure(self, client, sample_audio_file):
    ...
    def test_v1_inference_prediction_structure(self, client, sample_audio_file):
    ...
    def test_v1_inference_model_info_structure(self, client, sample_audio_file):
    ...
    def test_v1_inference_version_correct(self, client, sample_audio_file):
        """Test that response version is '1'."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert data["version"] == "1", \
            f"Version should be '1', got {data['version']}"

    def test_v1_inference_emotion_valid(self, client, sample_audio_file):
    ...
    def test_v1_inference_confidence_valid_range(self, client, sample_audio_file):
    ...
    def test_v1_inference_all_probabilities_valid(self, client, sample_audio_file):
    ...
    def test_v1_inference_probabilities_sum_to_one(self, client, sample_audio_file):
    ...
    def test_v1_inference_model_type_correct(self, client, sample_audio_file):
        """Test that model type is DecisionTreeClassifier."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert model_type == "DecisionTreeClassifier", \
            f"Model type should be DecisionTreeClassifier, got {model_type}"

    def test_v1_inference_features_used_correct(self, client, sample_audio_file):
        """Test that features_used is 162."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert features_used == 162, \
            f"Features used should be 162, got {features_used}"

    def test_v1_inference_processing_time_reasonable(self, client, sample_audio_file):
...
@pytest.mark.e2e
@pytest.mark.audio
class TestV1RealAudioEndpoint:
    """Test endpoint with real CREMA-D audio files."""

    def test_v1_inference_with_real_crema_d_audio(self, client, crema_d_sample):
    ...
    def test_v1_inference_multiple_real_files(self, client, crema_d_audio_path):
...
class TestV1ErrorHandling:
    ...
    def test_v1_inference_with_corrupted_file(self, client, corrupted_audio_file):
    ...
    def test_v1_inference_with_empty_file(self, client):
    ...
    def test_v1_inference_without_file(self, client):
        """Test that request without file returns 422 validation error."""
        response = client.post('/v1/infer/local/1')

        assert response.status_code == 422, \
            f"Request without file should return 422, got {response.status_code}"

    def test_v1_inference_with_invalid_file_type(self, client):
...
@pytest.mark.e2e
class TestV1FileFormatSupport:
    """Test support for different audio file formats."""

    def test_v1_inference_with_wav_file(self, client, sample_audio_file):
    ...
    def test_v1_inference_with_different_extensions(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV1EndpointConsistency:
    """Test endpoint consistency and reliability."""

    def test_v1_multiple_requests_same_audio(self, client, sample_audio_file):
    ...
    def test_v1_concurrent_requests_work(self, client, sample_audio_file):
...
@pytest.mark.e2e
class TestV1ModelInfoEndpoints:
    """Test model info endpoints for v1."""

    def test_get_model_info_v1(self, client):
        """Test GET /v1/models/local/1/info endpoint."""
        response = client.get('/v1/models/local/1/info')
        ...
        assert data["feature_dimension"] == 162, "Feature dimension should be 162"
        assert len(data["classes"]) == 6, "Should have 6 classes"

    def test_list_models_includes_v1(self, client):
        """Test GET /v1/models/local/list includes v1."""
        ...
        # Check v1 is in the list
        version_numbers = [v["version"] for v in data["versions"]]
        assert "1" in version_numbers, "v1 should be in versions list"

        # Find v1 in the list
        v1_info = next((v for v in data["versions"] if v["version"] == "1"), None)
        assert v1_info is not None, "v1 info should be in list"
        assert v1_info["model_type"] == "DecisionTreeClassifier"
        assert v1_info["feature_dimension"] == 162


@pytest.mark.e2e
class TestV1LatestEndpoint:
    """Test latest model endpoint (when v1 is latest)."""

    def test_latest_endpoint_works(self, client, sample_audio_file):
        """Test POST /v1/infer/local/latest endpoint."""
        files = {
            'file': ('test.wav', io.BytesIO(sample_audio_file), 'audio/wav')
        }
        ...
        assert data["version"] in ["1", "2", "3"], \
            "Latest version should be a valid version number"
