"""
Integration tests for Model v2 Full Inference Workflow.

This test suite validates the complete end-to-end inference flow for v2 model:
...
FEATURE_EXTRACTOR_PATH = (
    Path(__file__).resolve().parents[2] / "models" / "v2" / "feature_extractor.py"
)

spec = importlib.util.spec_from_file_location("v2_feature_extractor", FEATURE_EXTRACTOR_PATH)
v2_extractor_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(v2_extractor_module)
extract_features = v2_extractor_module.extract_features
...
class TestV2FullInferencePipeline:
    ...
    def test_full_pipeline_audio_to_prediction(self, registry, sample_audio_file):
        ...
        assert features.shape == (78,), "Features should have correct shape"

        # Step 2: Get model
        v2_model = registry.get_version("2")
        assert v2_model is not None, "v2 model should be available"

        # Step 3: Run prediction through registry
        result = registry.predict("2", sample_audio_file, "test.wav")
    ...
    def test_prediction_metadata_correct(self, registry, sample_audio_file):
        ...
        assert result["model_version"] == "2", "Model version should be '2'"
        assert (
            result["model_type"] == "Pipeline (StandardScaler + RFE + SVC)"
        ), "Model type should be Pipeline"
        assert result["feature_dimension"] == 78, "Feature dimension should be 78"
    ...
    def test_multiple_predictions_are_deterministic(self, registry, sample_audio_file):
        ...
        result1 = registry.predict("2", sample_audio_file, "test.wav")
        result2 = registry.predict("2", sample_audio_file, "test.wav")
...
@pytest.mark.integration
@pytest.mark.audio
class TestV2RealAudioInference:
    ...
    def test_real_crema_d_audio_inference(self, registry, crema_d_sample):
        ...
        # Run full inference
        result = registry.predict("2", audio_bytes, filename)
    ...
    def test_real_audio_produces_valid_features_and_prediction(self, registry, crema_d_sample):
        ...
        assert features.shape == (78,), "Should extract 78 features"
    ...
    def test_multiple_real_audio_files_all_produce_predictions(self, registry, crema_d_audio_path):
        ...
        for audio_file in audio_files:
            ...
            result = registry.predict("2", audio_bytes, audio_file.name)
...
@pytest.mark.integration
class TestV2RegistryPredictIntegration:
    ...
    def test_registry_predict_handles_feature_extraction(self, registry, sample_audio_file):
        ...
        assert result["feature_dimension"] == 78, "Should use correct feature dimension"
    ...
    def test_registry_predict_validates_features(self, registry):
        ...
        with pytest.raises(ValueError, match="Feature extraction failed"):
            registry.predict("2", invalid_audio, "invalid.wav")
    ...
    def test_registry_predict_with_different_filenames(self, registry, sample_audio_file):
        ...
        for filename in filenames:
            result = registry.predict("2", sample_audio_file, filename)
...
class TestV2ErrorHandlingIntegration:
    ...
    def test_none_audio_bytes_fails_gracefully(self, registry):
        ...
        with pytest.raises((ValueError, TypeError, AttributeError)):
            registry.predict("2", None, "none.wav")


@pytest.mark.integration
class TestV2PredictionConsistency:
    ...
    def test_different_audio_produces_predictions(self, registry):
        ...
        for i, audio_bytes in enumerate(audio_samples):
            result = registry.predict("2", audio_bytes, f"test_{i}.wav")
            predictions.append(result)
    ...
    def test_prediction_with_various_audio_characteristics(self, registry):
        ...
        for duration in durations:
            ...
            result = registry.predict("2", audio_bytes, f"test_{duration}s.wav")
...
@pytest.mark.integration
class TestV2GlobalRegistryIntegration:
    ...
    def test_global_registry_has_v2_registered(self):
        """Test that global registry has v2 model registered."""
        registry = get_registry()

        v2 = registry.get_version("2")
        assert v2 is not None, "v2 should be registered in global registry"

        info = registry.get_model_info("2")
        assert info is not None, "v2 info should be available"
        assert info["version"] == "2"


@pytest.mark.integration
class TestV2VsV1Comparison:
    """Test comparisons between v1 and v2 models."""

    def test_v2_and_v1_both_work_on_same_audio(self, registry, sample_audio_file):
        """Test that both v1 and v2 work on the same audio file."""
    ...
    def test_v2_uses_different_features_than_v1(self, registry, sample_audio_file):
        """Test that v2 extracts different features than v1."""
        v1_result = registry.predict("1", sample_audio_file, "test.wav")
        v2_result = registry.predict("2", sample_audio_file, "test.wav")
    ...
    def test_v2_has_different_model_architecture_than_v1(self, registry):
        """Test that v2 has different architecture than v1."""
