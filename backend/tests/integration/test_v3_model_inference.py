"""
Integration tests for Model v3 Full Inference Workflow.

This test suite validates the complete end-to-end inference flow for v3 model:
...
FEATURE_EXTRACTOR_PATH = (
    Path(__file__).resolve().parents[2] / "models" / "v3" / "feature_extractor.py"
)

spec = importlib.util.spec_from_file_location("v3_feature_extractor", FEATURE_EXTRACTOR_PATH)
v3_extractor_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(v3_extractor_module)
extract_features = v3_extractor_module.extract_features
...
class TestV3FullInferencePipeline:
    ...
    def test_full_pipeline_audio_to_prediction(self, registry, sample_audio_file):
        ...
        # Step 2: Get model
        v3_model = registry.get_version("3")
        assert v3_model is not None, "v3 model should be available"

        # Step 3: Run prediction through registry
        result = registry.predict("3", sample_audio_file, "test.wav")
    ...
    def test_prediction_all_probabilities_valid(self, registry, sample_audio_file):
        ...
        assert (
            predicted_emotion in all_probs
    ...
    def test_prediction_metadata_correct(self, registry, sample_audio_file):
        ...
        assert result["model_version"] == "3", "Model version should be '3'"
        assert (
            result["model_type"]
            == "Ultra Ensemble (Stacking + Extra Trees + Gradient Boosting + Majority Voting)"
        ), "Model type should be Ultra Ensemble"
        assert result["feature_dimension"] == 210, "Feature dimension should be 210"
    ...
    def test_multiple_predictions_are_deterministic(self, registry, sample_audio_file):
        ...
        result1 = registry.predict("3", sample_audio_file, "test.wav")
        result2 = registry.predict("3", sample_audio_file, "test.wav")
...
@pytest.mark.integration
@pytest.mark.audio
class TestV3RealAudioInference:
    ...
    def test_real_crema_d_audio_inference(self, registry, crema_d_sample):
        ...
        # Run full inference
        result = registry.predict("3", audio_bytes, filename)
    ...
    def test_multiple_real_audio_files_all_produce_predictions(self, registry, crema_d_audio_path):
        ...
        for audio_file in audio_files:
            ...
            result = registry.predict("3", audio_bytes, audio_file.name)
...
@pytest.mark.integration
class TestV3RegistryPredictIntegration:
    ...
    def test_registry_predict_validates_features(self, registry):
        ...
        with pytest.raises(ValueError, match="Feature extraction failed"):
            registry.predict("3", invalid_audio, "invalid.wav")
    ...
    def test_registry_predict_with_different_filenames(self, registry, sample_audio_file):
        ...
        for filename in filenames:
            result = registry.predict("3", sample_audio_file, filename)
...
@pytest.mark.integration
class TestV3ErrorHandlingIntegration:
    ...
    def test_none_audio_bytes_fails_gracefully(self, registry):
        ...
        with pytest.raises((ValueError, TypeError, AttributeError)):
            registry.predict("3", None, "none.wav")


@pytest.mark.integration
class TestV3PredictionConsistency:
    ...
    def test_different_audio_produces_predictions(self, registry):
        ...
        for i, audio_bytes in enumerate(audio_samples):
            result = registry.predict("3", audio_bytes, f"test_{i}.wav")
            predictions.append(result)
    ...
    def test_prediction_with_various_audio_characteristics(self, registry):
        ...
        for duration in durations:
            ...
            result = registry.predict("3", audio_bytes, f"test_{duration}s.wav")
...
@pytest.mark.integration
class TestV3GlobalRegistryIntegration:
    ...
    def test_global_registry_has_v3_registered(self):
        """Test that global registry has v3 model registered."""
        registry = get_registry()

        v3 = registry.get_version("3")
        assert v3 is not None, "v3 should be registered in global registry"

        info = registry.get_model_info("3")
        assert info is not None, "v3 info should be available"
        assert info["version"] == "3"


@pytest.mark.integration
class TestV3VsOtherVersionsComparison:
    """Test comparisons between v3 and other model versions."""

    def test_v3_v2_and_v1_all_work_on_same_audio(self, registry, sample_audio_file):
        """Test that v1, v2, and v3 all work on the same audio file."""
    ...
    def test_v3_uses_different_features_than_v1_and_v2(self, registry, sample_audio_file):
        """Test that v3 extracts different features than v1 and v2."""
        v1_result = registry.predict("1", sample_audio_file, "test.wav")
        v2_result = registry.predict("2", sample_audio_file, "test.wav")
        v3_result = registry.predict("3", sample_audio_file, "test.wav")
    ...
    def test_v3_has_different_model_architecture_than_v1_and_v2(self, registry):
        """Test that v3 has different architecture than v1 and v2."""
        v1_info = registry.get_model_info("1")
        v2_info = registry.get_model_info("2")
        v3_info = registry.get_model_info("3")

        assert (
            v1_info["model_type"] != v2_info["model_type"]
        ), "v1 and v2 should have different model types"
        assert (
            v2_info["model_type"] != v3_info["model_type"]
        ), "v2 and v3 should have different model types"
    ...
    def test_v3_produces_most_features(self, registry, sample_audio_file):
        """Test that v3 produces the most features among all versions."""
        v1_result = registry.predict("1", sample_audio_file, "test.wav")
        v2_result = registry.predict("2", sample_audio_file, "test.wav")
        v3_result = registry.predict("3", sample_audio_file, "test.wav")
        ...
        # v3 should have the most features
        assert v3_features == 210, "v3 should have 210 features"
        assert v3_features > v1_features, "v3 should have more features than v1"
        assert v3_features > v2_features, "v3 should have more features than v2"
