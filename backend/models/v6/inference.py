"""
SageMaker Inference Handler for Speech Emotion Recognition v6

This module provides inference functions for AWS SageMaker deployment
...
V6 Model Changes:
- Pure sklearn implementation (no UltraEnsemble)
- Model saved as dictionary: {'model': Pipeline, 'confusion_matrix_ensemble': ..., 'accuracy': 0.8223, ...}
- Pipeline: StandardScaler → StackingClassifier (7 base models + LogisticRegression meta-classifier)
...
def model_fn(model_dir: str):
    ...
    {
        'model': Pipeline(StandardScaler, StackingClassifier),
        'confusion_matrix_ensemble': ndarray,
        'confusion_matrix_components': ndarray,
        'class_labels': array(['angry', 'disgust', 'fear', 'happy', 'neutral', 'sad']),
        'accuracy': 0.8223
    }

    We extract the 'model' key which contains the sklearn Pipeline.
    ...
    try:
        ...
        # V6 models are saved as dictionaries
        if isinstance(pickled_data, dict):
            if 'model' not in pickled_data:
                raise ValueError("Dictionary structure missing 'model' key")

            model = pickled_data['model']

            # Add class_labels as classes_ attribute if not already present
            if 'class_labels' in pickled_data and not hasattr(model, 'classes_'):
                model.classes_ = pickled_data['class_labels']

            logger.info(
                f"Model loaded successfully: {type(model).__name__} "
                f"(extracted from dictionary, accuracy: {pickled_data.get('accuracy', 'N/A')})"
            )
        else:
            # Fallback for non-dictionary models
            model = pickled_data
...
def predict_fn(features: np.ndarray, model) -> Dict[str, Any]:
    ...
    Args:
        ...
        model: Loaded sklearn Pipeline from model_fn
