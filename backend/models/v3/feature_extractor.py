"""
Feature Extractor for Model v3 (Ultra Ensemble Model)
...
def extract_features(audio_bytes: bytes, filename: str) -> np.ndarray:
    """
    Extract 210 enhanced features from audio bytes for v3 model.
...
def extract_features_with_viz(audio_bytes: bytes, filename: str) -> Dict:
    """
    Extract 210 enhanced features AND intermediate visualization matrices from audio bytes.

    This function extracts the same 210 features as extract_features() but additionally
    captures intermediate matrices BEFORE averaging for frontend visualization:
    ...
    try:
        ...
        return {
            ...
            "visualization": visualization_data  # Dict with lists for JSON
        }

    except Exception as e:
        raise ValueError(f"Failed to extract features with visualization from audio: {str(e)}")
